package ru.ifmo.rain.Petrovski.implementor;
...
public class Implementor implements Impler, JarImpler {
    ...
    public static void main(String[] args) {
        ...
        if (args.length == 0) {
            System.err.println("Empty arguments list");
            return;
        }
        ...
        try {
            if (args[0].equals(JAR_OPTION)) {
                if (args.length == 3) {
                    ...
                    System.err.println("Wrong number of arguments, expected 3, found " + args.length);
                }
            } else {
                if (args.length == 2) {
                    implementor.implement(Class.forName(args[0]), Paths.get(args[1]));
                } else {
                    System.err.println("Wrong number of arguments, expected 2, found " + args.length);
                }
            }
        } catch (ClassNotFoundException e) {
            System.err.println("Problems with class name");
    ...
    private void compileClass(Path root, Path jarFilePath) throws ImplerException {
        ...
        try {
            ...
            throw new ImplerException("Error occurred when compiling", e);
        }
    }
